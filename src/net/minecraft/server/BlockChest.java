...
public class BlockChest extends BlockContainer {
    ...
    public boolean interact(World world, int i, int j, int k, EntityHuman entityhuman) {
        ...
        if (Block.o[world.getTypeId(i, j + 1, k)]) {
            return true;
        } else if (world.getTypeId(i - 1, j, k) == this.id && Block.o[world.getTypeId(i - 1, j + 1, k)]) {
            return true;
        } else if (world.getTypeId(i + 1, j, k) == this.id && Block.o[world.getTypeId(i + 1, j + 1, k)]) {
            return true;
        } else if (world.getTypeId(i, j, k - 1) == this.id && Block.o[world.getTypeId(i, j + 1, k - 1)]) {
            return true;
        } else if (world.getTypeId(i, j, k + 1) == this.id && Block.o[world.getTypeId(i, j + 1, k + 1)]) {
            return true;
        } else {
